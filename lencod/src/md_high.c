...
extern clock_t time_to_encode_intra_mode;
extern FILE* file;
...
void encode_one_macroblock_high (Macroblock *currMB)
{
  ...
  for (currMB->c_ipred_mode = chroma_pred_mode_range[0]; currMB->c_ipred_mode<=chroma_pred_mode_range[1]; currMB->c_ipred_mode++)
  {
    ...
    for (index=0; index < max_index; index++)
    {
      ...
      if (enc_mb.valid[mode])
      {
        ...
        // this is the added command to implement quick selection | show selected mode of I16MB & I4MB 
        if (mode == I16MB) {
            start_t = clock();
        } 
        ...
        if (PRINT_MODE_SELECT_INTRA) {
            if (mode == I4MB) {
                printf("%d\n", (short)(currMB->best_i16mode));
                fprintf(file, "%d\n", (short)(currMB->best_i16mode));
            }
        }
        if (INTRA4x4_EXIT_SOON) {
            currMB->intra4x4_exit_soon = 0;
        }
  ...
  time_to_encode_intra_mode += end_t - start_t;
