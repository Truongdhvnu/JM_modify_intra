...
//int get_i4x4_vertical_distortion_fast(Macroblock* currMB, imgpel* PredPel, int pic_pix_y, int pic_pix_x) {
//    imgpel** img_source = &currMB->p_Vid->enc_picture->imgY[pic_pix_y];
//    return i2(P_C - img_source[1][pic_pix_x + 1]) + i2(P_C - img_source[3][pic_pix_x + 1]) + 
//        i2(P_E - img_source[1][pic_pix_x + 3]) + i2(P_E - img_source[3][pic_pix_x + 3]);
//}
 ...
 ************************************************************************
 * \brief
 *    These are the added modes
 ************************************************************************
 */
static inline void mode_add_vertical_right(imgpel** cur_pred, imgpel* PredPel)
{
    cur_pred[0][0] =
        cur_pred[1][0] =
        cur_pred[2][0] =
        cur_pred[3][0] = (imgpel)((P_A * 3 + P_X + 2) >> 2);
    cur_pred[0][1] =
        cur_pred[1][1] =
        cur_pred[2][1] =
        cur_pred[3][1] = (imgpel)((P_B * 3 + P_A + 2) >> 2);
    cur_pred[0][2] =
        cur_pred[1][2] =
        cur_pred[2][2] =
        cur_pred[3][2] = (imgpel)((P_C * 3 + P_B + 2) >> 2);
    cur_pred[0][3] =
        cur_pred[1][3] =
        cur_pred[2][3] =
        cur_pred[3][3] = (imgpel)((P_D * 3 + P_C + 2) >> 2);
    //cur_pred[0][0] =
    //cur_pred[3][1] = P_A;
    //cur_pred[0][1] =
    //cur_pred[3][2] = P_B;
    ...
    //cur_pred[3][0] = P_L;
    //cur_pred[0][3] = P_D;
    ////cur_pred[1][0] = (5 * P_A + 3 * P_J + 4) >> 2;
    //cur_pred[1][0] = P_J;
    //cur_pred[1][1] = (5 * P_B + 3 * cur_pred[1][0] + 4) >> 3;
    //cur_pred[1][2] = (5 * P_C + 3 * cur_pred[1][1] + 4) >> 3;
    //cur_pred[1][3] = (5 * P_D + 3 * cur_pred[1][2] + 4) >> 3;
    ////cur_pred[2][0] = (5 * P_K + 3 * cur_pred[1][0] + 4) >> 3;
    //cur_pred[2][0] = P_K;
    //cur_pred[2][1] = (5 * cur_pred[2][0] + 3 * cur_pred[1][1] + 2) >> 3;
    //cur_pred[2][2] = (5 * cur_pred[2][1] + 3 * cur_pred[1][2] + 2) >> 3;
    //cur_pred[2][3] = (5 * cur_pred[2][2] + 3 * cur_pred[1][3] + 2) >> 3;
}

static inline void mode_add_vertical_left(imgpel** cur_pred, imgpel* PredPel) {
    cur_pred[0][0] =
        cur_pred[1][0] =
        cur_pred[2][0] =
        cur_pred[3][0] = (imgpel)((P_B * 3 + P_A + 2) >> 2);
    cur_pred[0][1] =
        cur_pred[1][1] =
        cur_pred[2][1] =
        cur_pred[3][1] = (imgpel)((P_C * 3 + P_B + 2) >> 2);
    cur_pred[0][2] =
        cur_pred[1][2] =
        cur_pred[2][2] =
        cur_pred[3][2] = (imgpel)((P_D * 3 + P_C + 2) >> 2);
    cur_pred[0][3] =
        cur_pred[1][3] =
        cur_pred[2][3] =
        cur_pred[3][3] = (imgpel)((P_E * 3 + P_D + 2) >> 2);
}

static inline void mode_add_horizontal_down(imgpel** cur_pred, imgpel* PredPel) {
    cur_pred[0][0] =
        cur_pred[0][1] =
        cur_pred[0][2] =
        cur_pred[0][3] = (imgpel)((P_X * 3 + P_I + 2) >> 2);
    cur_pred[1][0] =
        cur_pred[1][1] =
        cur_pred[1][2] =
        cur_pred[1][3] = (imgpel)((P_I * 3 + P_J + 2) >> 2);
    cur_pred[2][0] =
        cur_pred[2][1] =
        cur_pred[2][2] =
        cur_pred[2][3] = (imgpel)((P_J * 3 + P_K + 2) >> 2);
    cur_pred[3][0] =
        cur_pred[3][1] =
        cur_pred[3][2] =
        cur_pred[3][3] = (imgpel)((P_K * 3 + P_L + 2) >> 2);
}

static inline void mode_add_horizontal_up(imgpel** cur_pred, imgpel* PredPel) {
    cur_pred[0][0] =
        cur_pred[0][1] =
        cur_pred[0][2] =
        cur_pred[0][3] = (imgpel)((P_I * 3 + P_X + 2) >> 2);
    cur_pred[1][0] =
        cur_pred[1][1] =
        cur_pred[1][2] =
        cur_pred[1][3] = (imgpel)((P_J * 3 + P_I + 2) >> 2);
    cur_pred[2][0] =
        cur_pred[2][1] =
        cur_pred[2][2] =
        cur_pred[2][3] = (imgpel)((P_K * 3 + P_J + 2) >> 2);
    cur_pred[3][0] =
        cur_pred[3][1] =
        cur_pred[3][2] =
        cur_pred[3][3] = (imgpel)((P_L * 3 + P_K + 2) >> 2);
}
...
void get_intrapred_4x4(Macroblock* currMB, ColorPlane pl, int i4x4_mode, int img_x, int img_y, int left_available, int up_available)
{
    ...
    case HOR_UP_PRED:
        ...
        break;
    case MODE_ADD_VER_RIGHT:
        mode_add_vertical_right(curr_mpr_4x4[MODE_ADD_VER_RIGHT], PredPel);
        break;
    case MODE_ADD_VER_LEFT:
        mode_add_vertical_left(curr_mpr_4x4[MODE_ADD_VER_LEFT], PredPel);
        break;
    case MODE_ADD_HOR_RIGHT:
        mode_add_horizontal_down(curr_mpr_4x4[MODE_ADD_HOR_RIGHT], PredPel);
        break;
    case MODE_ADD_HOR_LEFT:
        mode_add_horizontal_up(curr_mpr_4x4[MODE_ADD_HOR_LEFT], PredPel);
        break;
    default:
...
int quick_intrapred_4x4(Macroblock* currMB, int pic_opix_y, int pic_pix_y, int pic_pix_x) {
    VideoParameters* p_Vid = currMB->p_Vid;
    distblk distortion = compute_SSE4x4(&p_Vid->pCurImg[pic_opix_y], &p_Vid->enc_picture->imgY[pic_pix_y], pic_pix_x, pic_pix_x);
    return distortion;
}
